...
- `ledger_verification`: the enforcement record written by the independent Ledger Verification Gate: `{verdict: "PASS"|"FAIL", spec_hash: "<sha256 of final spec bytes>", verifier_run_id, checked_at, blocks: [...]}`. Read by the PreToolUse ledger-gate hook, which refuses the autopilot/ralph/team bridge unless `verdict === "PASS"` with a matching `spec_hash`. See the Ledger State Model enforcement contract.
...
The Ledger Verification Gate writes `state.ledger_verification = { verdict: "PASS"|"FAIL", spec_hash: "<sha256 of the final spec file bytes>", verifier_run_id, checked_at, blocks: [...] }`. A PreToolUse hook refuses the execution-bridge `Skill()` call (autopilot / ralph / team / plan) whenever a deep-interview handoff is active unless `ledger_verification.verdict === "PASS"` AND `ledger_verification.spec_hash` equals the sha256 of the referenced spec at call time. A self-written PASS with no matching hash, or a spec edited after verification, is rejected by code.
...
When the verifier returns zero blocks, it WRITES the enforcement record: `state_write(mode="deep-interview")` with `state.ledger_verification = { verdict: "PASS", spec_hash: "<sha256 of the final spec file bytes>", verifier_run_id, checked_at, blocks: [] }` (on any block, write `verdict: "FAIL"` with the block list and return to Phase 2). Also record the verdict in the spec metadata. The PreToolUse **ledger-gate hook** (see the Ledger State Model enforcement contract) reads this record and refuses the autopilot/ralph/team bridge unless it is `PASS` with a `spec_hash` matching the current spec.
