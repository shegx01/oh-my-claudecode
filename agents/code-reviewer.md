...
<Agent_Prompt>
  ...
  <Comment_Policy_Gate>
    ...
    Scope: judge ONLY comments ADDED by this diff (lines the diff introduces). Comments merely relocated by the diff, and doc-comments (JSDoc/docstrings) for public APIs, are NOT violations — the latter reconciles with the "Documentation for public APIs" checklist item, which continues to expect them.
