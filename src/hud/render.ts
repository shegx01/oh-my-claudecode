...
export async function render(
  ...
  const safeMode =
    enabledElements.safeMode === true || process.platform === "win32";
  ...
  if (enabledElements.healthSigil) {
    const fiveHour =
      context.rateLimitsResult?.rateLimits?.fiveHourPercent ?? null;
  ...
  if (enabledElements.githubUser) {
    ...
    if (user) rendered.set("githubUser", user);
  }
  ...
  if (enabledElements.cwd) {
    if (enabledElements.cwdFormat === "last") {
      ...
      const cwdElement = renderStackedCwd(
        context.cwd,
        branch,
        worktreeName,
        safeMode,
  ...
  if (enabledElements.effort) {
    const effortElement = renderEffort(
  ...
  if (showCounts) {
    ...
    // does not strip ⚙).
    const iconOverrides =
      useDotMeters && !safeMode ? { tool: '⚙' } : undefined;
  ...
  if (useDotMeters) {
    ...
    if (activity) renderedDetail.set("activity", [activity]);
  }
  ...
  function collectInlinePairs(order: string[]): Array<[string, string]> {
    ...
    for (const name of order) {
      ...
      if (el) {
        ...
        const lines = renderedDetail.get(name);
        if (lines && lines.length > 0) result.push([name, lines.join(" ")]);
      }
    }
    return result;
  }
   ...
   */
  function joinStackedLine(pairs: Array<[string, string]>): string {
