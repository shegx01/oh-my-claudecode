 ...
 */
export const ledgerVerifyTool: ToolDefinition<{
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const diState = readModeState<Record<string, unknown>>('deep-interview', root, sessionId || undefined);
      if (!diState) {
        return {
          content: [{ type: 'text' as const, text: 'ledger_verify error: no active deep-interview state found for this session.' }],
          isError: true,
        };
      }
      ...
      if (!specPath) {
        return {
          content: [{ type: 'text' as const, text: 'ledger_verify error: no spec_path provided and none present in deep-interview state.' }],
          isError: true,
        };
      }
      ...
      if (!written) {
        return {
          content: [{ type: 'text' as const, text: 'ledger_verify error: could not persist the ledger-verification record (state mutation lock unavailable).' }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: 'text' as const, text: `ledger_verify error: ${error instanceof Error ? error.message : String(error)}` }],
        isError: true,
      };
    }
  }
