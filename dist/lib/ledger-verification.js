...
function hasPersistedUserExitTurn(rounds) {
    return rounds.map(asRecord).some((round) => {
        if (!round)
            return false;
        if (round.user_exit === true || round.early_exit === true)
            return true;
...
export function evaluateLedgerVerification(diState) {
    const blocks = [];
    ...
    for (const row of ledger) {
        if (rowStatus(row) !== 'pending_consensus')
            continue;
...
//# sourceMappingURL=ledger-verification.js.map